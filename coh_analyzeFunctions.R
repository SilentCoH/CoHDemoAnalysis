...
analyzeGroupDefense <- function(parseOut, teamNum,
                                evadeset = c("Phase Shift","Hibernate","Jaunt","Raptor or Fly")){
  if (teamNum==0){
...
analyzeCustomGroupOffense <- function(parseOut, name1, name2, name3=NULL, name4=NULL, name5=NULL, name6=NULL, name7=NULL, name8=NULL){
  ...
  datOut<-as.data.frame(matrix(NA,nrow=validPlayers,ncol=8))
  ...
  names(datOut)<-c("name",names(p1)[2:8])
  datOut[1,"name"] <- p1$rawData$name[1]
  datOut[1,2:8] <- as.numeric(p1[2:8])
  datOut[2,"name"] <- p2$rawData$name[1]
  datOut[2,2:8] <- as.numeric(p2[2:8])
  if (!is.null(name3)){ datOut[3,"name"] <- p3$rawData$name[1]; datOut[3,2:8] <- as.numeric(p3[2:8]) }
  if (!is.null(name4)){ datOut[4,"name"] <- p4$rawData$name[1]; datOut[4,2:8] <- as.numeric(p4[2:8]) }
  if (!is.null(name5)){ datOut[5,"name"] <- p5$rawData$name[1]; datOut[5,2:8] <- as.numeric(p5[2:8]) }
  if (!is.null(name6)){ datOut[6,"name"] <- p6$rawData$name[1]; datOut[6,2:8] <- as.numeric(p6[2:8]) }
  if (!is.null(name7)){ datOut[7,"name"] <- p7$rawData$name[1]; datOut[7,2:8] <- as.numeric(p7[2:8]) }
  if (!is.null(name8)){ datOut[8,"name"] <- p8$rawData$name[1]; datOut[8,2:8] <- as.numeric(p8[2:8]) }
...
analyzeGroupOffense <- function(parseOut, teamNum){
  ...
  datOut[2,"name"] <- p2$rawData$name[1]
  datOut[2,2:8] <- as.numeric(p2[2:8])
  if (!is.null(p3)){ datOut[3,"name"] <- p3$rawData$name[1]; datOut[3,2:8] <- as.numeric(p3[2:8]) }
  if (!is.null(p4)){ datOut[4,"name"] <- p4$rawData$name[1]; datOut[4,2:8] <- as.numeric(p4[2:8]) }
  if (!is.null(p5)){ datOut[5,"name"] <- p5$rawData$name[1]; datOut[5,2:8] <- as.numeric(p5[2:8]) }
  if (!is.null(p6)){ datOut[6,"name"] <- p6$rawData$name[1]; datOut[6,2:8] <- as.numeric(p6[2:8]) }
  if (!is.null(p7)){ datOut[7,"name"] <- p7$rawData$name[1]; datOut[7,2:8] <- as.numeric(p7[2:8]) }
  if (!is.null(p8)){ datOut[8,"name"] <- p8$rawData$name[1]; datOut[8,2:8] <- as.numeric(p8[2:8]) }
  datOut$compactness <- datOut$chain_duration / datOut$attacks_per_spike
  return(datOut)
}
